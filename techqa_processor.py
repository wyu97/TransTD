...
class TechQaInputFeature(object):
    ...
    __slots__ = ['qid', 'doc_id', 'doc_span_index', 'tokens', 'token_to_original_start_offset',
                 'token_to_original_end_offset', 'input_ids', 'input_mask', 'segment_ids',
                 'start_position', 'end_position', 'snippet_length', 'document_pair']
    ...
    def __init__(self, qid: str, doc_id: str, doc_span_index: int, tokens: List[str],
        ...
        self.snippet_length = end_position - start_position

        if self.snippet_length > 0:
            self.document_pair = 1
        else:
            self.document_pair = 0
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    return  get_tech_qa_features(
                            ...
                            use_old_cached_file=args.use_cached_file
                        )
...
def get_dataset_from_features(features):
    ...
    all_snippet_lengths = torch.tensor([f.snippet_length for f in features], dtype=torch.long)
    all_document_pairs = torch.tensor([f.document_pair for f in features], dtype=torch.long)
    ...
    dataset = TensorDataset(
        all_input_ids, all_input_mask, all_segment_ids, all_start_positions, \
            all_end_positions, all_snippet_lengths, all_document_pairs, all_feature_indices)
...
def get_tech_qa_features(
        ...
        use_old_cached_file: bool = False,
        ) -> \
    ...
    if use_old_cached_file and os.path.exists(cache_file):
